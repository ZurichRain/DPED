...
class JointThreeLinkMutiviewerPromptMutiDistDataset(Dataset):
    ...
    def preprocess(self,oridocdic,docid2docname):
        ...
        confidence_prompt_seqs_lis=[]
        confidence_prompt_labels_lis=[]
        ...
        for k,v in self.oridocdic.items():
            ...
            confidence_prompt_seqs_lis+=v.confidence_prompt_seqs
            confidence_prompt_labels_lis+=v.confidence_prompt_labels

        data=[]
        for seq,e1msk,e2msk,trmsk,lab,conf_seq,conf_lab in zip(all_link_seq_lis,all_e1_mask_lis,all_e2_mask_lis,all_tr_mask_lis,all_lab_lis,\
                                            confidence_prompt_seqs_lis,confidence_prompt_labels_lis):
            data.append((seq,e1msk,e2msk,trmsk,lab,conf_seq,conf_lab))
        return data
    ...
    def __getitem__(self, idx):
        ...
        conf_seq = self.dataset[idx][5]
        conf_lab = self.dataset[idx][6]
        return [seq,e1msk,e2msk,trmsk,lab,conf_seq,conf_lab]
    ...
    def get_link_one_doc(self,doc,char2tok_spanlis):
        ...
        confidence_prompt_seqs = []
        confidence_prompt_labels = []
        ...
        for seqidx in range(len(doc.all_link_seqs)):
            ...
            confidence_prompt_token=self.tokenizer.convert_tokens_to_ids(\
                    self.tokenizer.tokenize('In this sentence')
                )\
                +self.tokenizer.convert_tokens_to_ids(\
                self.tokenizer.tokenize('[MASK] '*len(doc.all_link_seqs[seqidx][doc.all_tr_sted[seqidx][0]:doc.all_tr_sted[seqidx][1]+1])+'can connect')
                )\
                +doc.all_link_seqs[seqidx][doc.all_e1_sted[seqidx][0]:doc.all_e1_sted[seqidx][1]+1]\
                + self.tokenizer.convert_tokens_to_ids(\
                    self.tokenizer.tokenize('and')
                ) \
                +doc.all_link_seqs[seqidx][doc.all_e2_sted[seqidx][0]:doc.all_e2_sted[seqidx][1]+1]
            
            # if(doc.all_link_label[seqidx]==1):
            confidence_prompt_lab_token =  self.tokenizer.convert_tokens_to_ids(\
                    self.tokenizer.tokenize('In this sentence')
                )\
                + doc.all_link_seqs[seqidx][doc.all_tr_sted[seqidx][0]:doc.all_tr_sted[seqidx][1]+1]+\
                self.tokenizer.convert_tokens_to_ids(\
                    self.tokenizer.tokenize('can connect')
                )\
                +doc.all_link_seqs[seqidx][doc.all_e1_sted[seqidx][0]:doc.all_e1_sted[seqidx][1]+1]\
                + self.tokenizer.convert_tokens_to_ids(\
                    self.tokenizer.tokenize('and')
            ...
            if(len(doc.all_link_seqs[seqidx])+len(prompt_token)>=512):
                ...
                dis_cofidence = len(doc.all_link_seqs[seqidx])+len(confidence_prompt_token) - 512
                content_token = doc.all_link_seqs[seqidx]
                doc.all_link_seqs[seqidx] = doc.all_link_seqs[seqidx][:len(doc.all_link_seqs[seqidx])-dis] + prompt_token
                if(dis_cofidence >= 0):
                    confidence_prompt_seqs.append(content_token[:len(doc.all_link_seqs[seqidx])-dis_cofidence]+confidence_prompt_token)
                    confidence_prompt_labels.append(content_token[:len(doc.all_link_seqs[seqidx])-dis_cofidence]+confidence_prompt_lab_token)
                else:
                    confidence_prompt_seqs.append(content_token+confidence_prompt_token)
                    confidence_prompt_labels.append(content_token+confidence_prompt_token)
            else:
                dis_cofidence = len(doc.all_link_seqs[seqidx])+len(confidence_prompt_token) - 512
                content_token = doc.all_link_seqs[seqidx]
        ...
        doc.confidence_prompt_seqs = confidence_prompt_seqs
        doc.confidence_prompt_labels = confidence_prompt_labels
    ...
    def collate_fn(self, batch):
        ...
        conf_seqs = [x[5] for x in batch]
        conf_labs = [x[6] for x in batch]
        ...
        max_conf_len = max([len(s) for s in conf_seqs])
        batch_conf_data=[[0 for i in range(max_conf_len)]for j in range(batch_len)]
        for j in range(batch_len):
            cur_len = len(conf_seqs[j])
            batch_conf_data[j][:cur_len] = conf_seqs[j]

        batch_conf_labs=[[0 for i in range(max_conf_len)]for j in range(batch_len)]
        for j in range(batch_len):
            cur_len = len(conf_labs[j])
            batch_conf_labs[j][:cur_len] = conf_labs[j]
        ...
        batch_conf_data = torch.tensor(batch_conf_data,dtype=torch.long).to(self.device)
        batch_conf_labs = torch.tensor(batch_conf_labs,dtype=torch.long).to(self.device)
        ...
        return {
            ...
            'conf_seqs':batch_conf_data,
            'conf_labs':batch_conf_labs,
        }
...
if __name__ == '__main__':
    ...
    train_dataset = JointThreeLinkMutiviewerPromptMutiDistDataset(processor.train_oridocdic,processor.train_docid2docname, config)
