...
def train_epoch(train_loader, model, optimizer, epoch , eval_fun ,tr_mask_tok,scheduler=None,train_writer=None):
    ...
    for idx, batch_samples in enumerate(tqdm(train_loader)):
        ...
        loss,prey,mask_token_index = model(**cbatch)

        mask_logits = prey[mask_token_index]
        mask_logits += tr_mask_tok
        prey1_lab=torch.argmax(mask_logits,dim=-1)
        
        prey1_lis += prey1_lab.to('cpu').numpy().tolist()
        
        truy1_lis += cbatch['y_labels'][mask_token_index].view(-1).to('cpu').numpy().tolist()
...
def train_MLM( model:torch.nn.Module, optimizer,train_loader, eval_fun,tr_mask_tok=None,dev_loader=None, scheduler=None, model_dir=None,train_writer=None,test_writer=None):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        train_epoch(train_loader, model, optimizer, epoch ,eval_fun ,tr_mask_tok, scheduler,train_writer)
        
        if dev_loader is None:
            pass
        else:
            # pass
            val_metrics = evaluate(dev_loader, model,epoch,eval_fun,tr_mask_tok,test_writer=test_writer)
...
def evaluate(dev_loader, model, epoch,eval_fun,tr_mask_tok,mode='dev',test_writer=None):
